...
function createWordPressDatabase($siteId, $dbConfig) {
    ...
    $result = executeDockerCommand("exec webbadeploy_db mysql -u root -pwebbadeploy_root_pass -e \"$sql\"");
    if (!$result['success']) {
...
function deleteSiteById($db, $id) {
    try {
        ...
        if ($site['ssl']) {
            executeDockerCommand("exec webbadeploy_nginx certbot delete --cert-name {$site['domain']} --non-interactive");
        }
