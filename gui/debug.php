...
$debugEnabled = file_exists('/tmp/webbadeploy_debug_enabled');

// Handle enable/disable toggle
if (isset($_POST['toggle_debug'])) {
    if ($debugEnabled) {
        unlink('/tmp/webbadeploy_debug_enabled');
        $debugEnabled = false;
        $message = 'Debug mode disabled';
    } else {
        file_put_contents('/tmp/webbadeploy_debug_enabled', '1');
        $debugEnabled = true;
