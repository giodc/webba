#!/bin/bash
# Quick fix for port conflict during updates
# This patches the upgrade.sh script with the port release fix

UPGRADE_SCRIPT="/opt/wharftales/scripts/upgrade.sh"

if [ ! -f "$UPGRADE_SCRIPT" ]; then
    echo "Error: upgrade.sh not found at $UPGRADE_SCRIPT"
    exit 1
fi

echo "Backing up current upgrade.sh..."
cp "$UPGRADE_SCRIPT" "${UPGRADE_SCRIPT}.backup-$(date +%Y%m%d-%H%M%S)"

echo "Checking if fix is already applied..."
if grep -q "docker-compose down" "$UPGRADE_SCRIPT"; then
    echo "✓ Fix already applied!"
    exit 0
fi

echo "Applying port conflict fix..."

# Create the fixed version
cat > "$UPGRADE_SCRIPT" << 'EOFSCRIPT'
#!/bin/bash
...
echo ""
echo "The upgrade script now:"
echo "  1. Stops containers before updating (releases ports)"
echo "  2. Waits for ports to be released"
echo "  3. Verifies port 80 is free before starting"
echo "  4. Starts containers cleanly"
echo ""
echo "You can now run updates without port conflicts."
